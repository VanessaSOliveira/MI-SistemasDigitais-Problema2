	...
	unsigned long status = 0;
	  ...
	  //  while((status & 0x00000040) != 0x00000040)
	    //{
	        ...
	        //printf("Status tx loop - %x", status);
	   // }
	    //status = IORD_ALTERA_AVALON_UART_STATUS(UART_0_BASE);
	        //printf("Status tx fora do  abaixo - %x", status);
	    char data ="";
while(1){
	    for (i = 0; i < tamanho; i++) {
	    	IOWR_ALTERA_AVALON_UART_TXDATA(UART_0_BASE, comando[i]);
	    	//char c = IORD(UART_0_BASE, ALTERA_AVALON_UART_TXDATA_REG);
	    	//printf("caracter do tx %c -- ", c);
	    	//usleep(1000);
	        //status = IORD_ALTERA_AVALON_UART_STATUS(UART_0_BASE);
	        //printf("Status tx loop - %x", status);
	    }
	    ...
	    char a = IORD(UART_0_BASE, ALTERA_AVALON_UART_TXDATA_REG);
	    printf("caracter do tx\%ca -- ", a);
    	...
    	//
	    	if(IORD_ALTERA_AVALON_UART_STATUS(UART_0_BASE) & 0x60){
